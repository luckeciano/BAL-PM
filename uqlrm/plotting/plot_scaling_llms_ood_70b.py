...
exp_names = ['rand_adp_rank_bald_reddit_70b', 'ep_adp_rank_bald_reddit_70b', 'batchstateentropy_adp_rank_bald_reddit_70b_7bstfts'] #, 'al_batchent_nounc']
legend_dict = {
    'rand_adp_rank_bald_reddit_70b': 'Random Sampling',
    'ep_adp_rank_bald_reddit_70b': 'BALD',
    'batchstateentropy_adp_rank_bald_reddit_70b_7bstfts': 'BAL-PM (ours)'
}
...
for i, exp_name in enumerate(exp_names):
    ...
    if legend_dict[exp_name] == 'Random Sampling':
        ...
        line1 = plt.Line2D([xstmark, xstmark], [-0.615, final_rnd_perf], color='black', linestyle='--')
        line2 = plt.Line2D([xenmark, xenmark], [-0.615, final_rnd_perf], color='black', linestyle='--')
        ...
        ax.scatter([xstmark, xenmark], [-0.615, -0.615], color='black', s=30)
        ...
        ax.annotate(text=' ',xy=(xstmark, -0.615), xycoords='data',
				xytext=(xenmark + 320, -0.615),textcoords='data',
				arrowprops=dict(arrowstyle="<->", color='black', linewidth=1.5), va='center')
        ax.annotate(text='~32% less samples',xy=(((xstmark+xenmark)/2), -0.615 + 0.0007), xycoords='data',fontsize=15.0,textcoords='data',ha='center')
...
# ax.set_ylim(-0.64, -0.595)

ax.legend(loc='lower right')
plt.tight_layout()
# plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)
# Save the figure in EPS, PDF, and PNG formats
output_name = 'scaling_llms_70b_ood_figure'
