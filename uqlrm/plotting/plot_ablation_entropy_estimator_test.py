...
exp_names = ['baepmv2_rank_adp_reddit_7b_base', 'baepm_reddit_7b_ratiolog_final'] #, 'al_batchent_nounc']
legend_dict = {
    'baepmv2_rank_adp_reddit_7b_base': 'KL Estimator',
    'baepm_reddit_7b_ratiolog_final': 'KSG Estimator (ours)'
}
...
for i, exp_name in enumerate(exp_names):
    ...
    if exp_name == 'baepm_reddit_7b_ratiolog_final':
        ax.plot(x, mean, label=legend_dict[exp_name], color=palette[2])
        ax.fill_between(x, mean-stderr, mean+stderr, alpha=0.3, color=palette[2])
    else:
        ax.plot(x, mean, label=legend_dict[exp_name], color=palette[3])
        ax.fill_between(x, mean-stderr, mean+stderr, alpha=0.3, color=palette[3])

    if legend_dict[exp_name] == 'KL Estimator':
        final_rnd_perf = mean.values[-1]
    elif legend_dict[exp_name] == 'KSG Estimator (ours)':
        perf_values = mean.values
        index = np.argmax(perf_values > final_rnd_perf)
        xstmark = index * 320
        xenmark = len(mean) * 320
        print(f"Sample Gain Ratio: {1 - (xstmark / xenmark)}")

        line1 = plt.Line2D([xstmark, xstmark], [-0.62, final_rnd_perf], color='black', linestyle='--')
        line2 = plt.Line2D([xenmark, xenmark], [-0.62, final_rnd_perf], color='black', linestyle='--')
        ...
        ax.scatter([xstmark, xenmark], [-0.62, -0.62], color='black', s=30)
        ...
        ax.annotate(text=' ',xy=(xstmark, -0.62), xycoords='data',
				xytext=(xenmark + 320, -0.62),textcoords='data',
				arrowprops=dict(arrowstyle="<->", color='black', linewidth=1.5), va='center')
        ax.annotate(text='~20% fewer samples',xy=(((xstmark+xenmark)/2), -0.62 - 0.001), xycoords='data',fontsize=13.0,textcoords='data',ha='center')
...
ax.set_yticks([-0.61, -0.62, -0.63])
# ax.set_yticklabels([-0.62, -0.61])

ax.set_xlim(4000, 25000)
ax.set_ylim(-0.63, -0.6075)

ax.legend(loc='lower right')
# Get the legend
legend = ax.legend_

# Loop over the legend handles (the color identifiers)
for handle in legend.legendHandles:
   handle.set_linewidth(3)  # Set the size to be larger

plt.tight_layout()
# plt.subplots_adjust(left=0.1, right=0.9, top=0.9, bottom=0.1)
# Save the figure in EPS, PDF, and PNG formats
output_name = 'ablation_entropy_estimator_test_figure'
