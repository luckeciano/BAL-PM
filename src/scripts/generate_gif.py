...
def make_gif(args):
    ...
    for ckpt in ckpts:
        filename = os.path.join('./images', args.experiment_name, f"{args.experiment_prefix}_{args.experiment_name}_ckpt_{ckpt}.jpg")
        frames.append(Image.open(filename))
    frame_one = frames[0]
    gif_path = os.path.join('./animations', f"{args.experiment_prefix}_{args.experiment_name}.gif")
    frame_one.save(gif_path, format="GIF", append_images=frames, save_all=True, duration=500, loop=0)

def generate_gif(args):
    ckpts = [1] + list(range(args.min_ckpt, args.max_ckpt, args.steps_ckpt))
    with imageio.get_writer(os.path.join('./animations', f"{args.experiment_prefix}_{args.experiment_name}.gif"), mode='I', duration=2) as writer:     
        for ckpt in ckpts:
            filename = os.path.join('./images', args.experiment_name, f"{args.experiment_prefix}_{args.experiment_name}_ckpt_{ckpt}.eps")
